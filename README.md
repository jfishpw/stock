...
**重要说明（2025年更新）**：系统现已支持多数据源，优先使用新浪财经数据源获取核心股票数据（实时行情、历史K线等），当新浪数据源不可用时自动切换到东方财富网。因此Cookie设置变为可选项。
...
**多数据源支持说明**：
- 系统默认使用新浪财经数据源获取A股实时行情和历史K线数据
- 当主要数据源不可用时，系统会自动切换到备用数据源
- 数据源状态会自动监控，故障恢复后会自动重新启用
- 这种设计大大提高了数据获取的稳定性和可靠性
### 9.安装自动交易（可选）
